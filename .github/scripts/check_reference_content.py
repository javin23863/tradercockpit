...
def validate_content(root):
    ...
    try:
        ...
        for name,row in m['files'].items():
            ...
            restored=unwrap_html(text) if row['kind'] in {'reading_shell','corrected_reading_shell'} else text.replace(GUARD,'',1).replace(ESCAPE_BLOCK,'',1) if row['kind']=='search_guard' else None
            if row['kind']=='ux_contracts':
                ...
                if not isinstance(row.get('correction'),str) or not row['correction'].strip():errors.append('Declared reading correction missing reason: '+name)
                if restored is None or sha(restored.encode('utf-8'))!=row.get('corrected_unwrapped_sha256'):errors.append('Declared reading correction differs: '+name)
