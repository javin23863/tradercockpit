# E1 Thumbnail Brief

## Working package

- Title: **You Don’t Have a Trading Strategy — You Have a Backtest**
- Thumbnail words: **DON’T TRADE THIS**
- Spoken confirmation: **“An indicator and a green backtest do not make a trading strategy. They make a candidate.”**
- External media cost: $0

The title confronts the viewer’s category error. The thumbnail gives the concrete money boundary. The spoken line identifies the missing classification. The fixed ident owns `YOU ARE THE MARKET`; the thumbnail does not repeat it.

## Composition

- Operator occupies the left side at roughly half the frame, looking toward the evidence.
- Operator holds a printed green equity curve that has been stamped **CANDIDATE** in red.
- Thumbnail words occupy the right side in large warm-white type, with **TRADE** in `#FF1744`.
- Background is `#08030a` with the TraderCockpit gauge grid and fixed logo in the bottom-right corner.
- Palette: black, warm white, red. Green appears only inside the backtest sheet as semantic bait.
- Expression: controlled disbelief, not fake shock.

Visual element count: operator, stamped backtest, text.

## Mobile and honesty checks

- Downscale to 140 px before approval.
- The words remain readable without the stamp; the stamp remains readable without the words.
- Keep the text out of the bottom overlay zone.
- The curve is a clearly designed prop, not a claimed real performance artifact.
- Do not add money, percentages, profit claims, luxury imagery, or a guru face.

## Intro candidates

The selected opening is in `vo.txt`. Film several clean alternatives and choose during the read-aloud pass:

- “A green backtest is where research starts. It is not where a strategy finishes.”
- “If your strategy began with an indicator, the chart probably wrote the idea for you.”
- “You did not build a trading strategy. You selected a result from old prices.”
- “The most dangerous word in retail trading is ‘strategy.’ Most traders use it far too early.”
- “Your backtest can be completely accurate and still give you no authority to risk money.”
